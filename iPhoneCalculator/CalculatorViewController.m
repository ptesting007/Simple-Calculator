...
//    [self addButtonWithTitle:@"mr"      tag:kButtonMR           row:0 column:3 width:1 height:1];
    ...
    [self addButtonWithTitle:@"AC"      tag:kButtonAC           row:1 column:0 width:2 height:1];
...
- (void)doOperation:(int)theOperation
{
    ...
    self.resultText = [self readableNumberFromResult:[NSMutableString stringWithFormat:@"%f", result]];
}
...
- (NSMutableString *)readableNumberFromResult:(NSString *)aString
{
...
- (void)buttonPressed:(id)sender
{
    ...
    if ([sender tag] == kButtonDot) {
        if ([self.resultText rangeOfString:@"."].location == NSNotFound) {
